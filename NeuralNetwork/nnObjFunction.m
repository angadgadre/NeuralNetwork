...
%     for j = 1 : n_hidden
%         for i = 1:n_input
%             a(j) = a(j) + w1(j,i)*training_data2(N,i);
%         end
%         a(j) = a(j) + w1(j, (n_input + 1)) * 1;
%     end
...
%            b(k) = b(k) + w2(k,j)*z(j);
%         end
%         b(k) = b(k) + w2(k, n_hidden + 1) * 1;
%     end
    
    b = w2 * z;
    y = sigmoid(b);
    
    delta = zeros(n_class, 1);
    err_fn_n = zeros(n_class, 1);
    grad_w2_n = zeros(n_class, (n_hidden + 1));
    for k = 1 : n_class
        delta(k) = y(k) - t(N, k);
        err_fn_n(k) = -(t(N,k)*log(y(k)) + (1-t(N,k))*log(1-y(k)));
        for j = (1:n_hidden + 1)
            grad_w2_n(k, j) = delta(k) * z(j);
        end
    end
    err_fn = err_fn + sum(err_fn_n);
